...
- **Data fetching**:
  - L'app router utilizza l'architettura RSC che permette di fetchare dati utilizzando sia server components che client components.
    - Utilizzo dei server components: è vantaggioso perché questi hanno accesso diretto alle risorse lato server (come database o file system) e supportano varie configurazioni per caching e rivalidazione. Questo minimizza inoltre la necessità di calcoli lato client.
      ...
      - Data Cache: di default, Next.js cacha i valori restituiti dalle fetch in una cache HTTP lato server che mantiene il risultato delle data fetch tra le richieste in arrivo. Questo serve per migliorare le prestazioni e ridurre i costi eliminando la necessità di ri-fetchare i dati dalla fonte ad ogni richiesta.
        - I dati della cache si trovano in .next/cache/fetch-cache.
        - La cache si può disabilitare per le singole data fetch utilizzando l'opzione { cache: "no-store" }, ma una volta specificata questa opzione per una fetch, anche le richieste successive non saranno cachate, perciò è bene specificare le fetch che devono utilizzare la cache prima di quelle che non la devono utilizzare. Un'alternativa all'opzione cache è la route segment configuration, che prevede di indicare all'inizio di layout o di page il seguente codice: export const fetchCache = "default-cache"; in modo da definire il comportamento della cache a livello di route.
        ...
        - La rivalidazione della cache permette di aggiornare la Data Cache ottenendo i dati più recenti. Un esempio è la rivalizazione basata sul tempo, la cui frequenza riguardo un'intera route è data dal minimo tempo di rivalidazione indicato tra layout e page.
        - Il tempo di rivalidazione per una fetch si può specificare in secondi utilizzando l'opzione { revalidate: 10 }. Un'alternativa all'opzione revalidate è la route segment configuration, che prevede di indicare all'inizio di layout o di page il seguente codice: export const revalidate = 10; in modo da definire il te,tempo di rivalidazione della cache a livello di route.
      - Memoizzazione delle richieste: funzionalità che deduplica le richieste per gli stessi dati in un singolo passaggio di render. Questo permette di riutilizzare dati nell'albero di componenti React (al posto di passarli tramite props a partire dall'alto dell'albero), prevenire chiamate di rete ridondanti e migliorare le prestazioni: la richiesta iniziale ottiene i dati da una fonte esterna, quindi i dati vengono salvati in memoria così da essere riutilizzati dalle richieste successive.
        - La memoizzazione è di React, non solo di Next.js nello specifico.
        - La memoizzazione viene applicata solo al metodo GET nelle richieste fetch. Inoltre, viene applicata solo nell'albero di componenti React, quindi non si estende alle richieste fetch nei gestori di route.
        - Nei casi in cui la fetch non è adatta (clients di database, di CMS, di GraphQL), si può usare la funzione cache di React per memoizzare funzioni.
    - Utilizzo dei client components: solitamente viene gestito tramite librerie di terze parti, come ad esempio TanStack Query, in quanto out of the box non si hanno funzionalità di memoizzazione, caching e rivalidazione.

Fatto fino al [video 71](https://www.youtube.com/watch?v=PlgDOhWFOno&list=PLC3y8-rFHvwjOKd6gdf4QtV1uYNiQnruI&index=71) compreso.
